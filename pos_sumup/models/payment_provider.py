    ...
    def sumup_make_request(self, endpoint, data=None, method='POST', params=None):
        ...
        try:
            ...
            try:
                ...
                _logger.error("SumUp API HTTP Error: %s", error_data)
                errors = error_data.get('errors') if isinstance(error_data, dict) else None
                if isinstance(errors, dict):
                    error_msg = errors.get('detail') or errors.get('message')
                    error_code = errors.get('type') or errors.get('code')
                else:
                    error_msg = None
                    error_code = None
                error_msg = (
                    error_msg
                    or (error_data.get('message') if isinstance(error_data, dict) else None)
                    or (error_data.get('error_description') if isinstance(error_data, dict) else None)
                    or str(e)
                )
                error_code = (
                    error_code
                    or (error_data.get('error_code') if isinstance(error_data, dict) else None)
                    or 'API_ERROR'
                )
